...
function SectionTabs({ tabs, activeTab, onChange, isIcons = true ,className}) {
  return (
    <div className="flex flex-wrap justify-between lg:justify-center lg:gap-2 bg-lightGray md:w-fit border-b-2 border-neutral_variant-20">
      {tabs.map((tab) => (
        <button
          key={tab.id}
          type="button"
          onClick={() => onChange?.(tab.id)}
          className={`relative flex items-center gap-1.5 px-3 py-1.5 text-xs font-normal label-m transition ${className} ${activeTab === tab.id
