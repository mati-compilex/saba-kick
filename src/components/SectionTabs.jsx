...
function SectionTabs({ tabs, activeTab, onChange }) {
  ...
  return (
    <div className="flex flex-wrap justify-between lg:justify-center lg:gap-2 bg-lightGray rounded-[16px] p-1">
      {tabs.map((tab) => (
        <button
          key={tab.id}
          type="button"
          onClick={() => onChange?.(tab.id)}
          className={`flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-normal transition ${
            activeTab === tab.id
